...
def create_access_token(data: dict, expires_delta: Optional[timedelta] = None):
    ...
    encode_jwt = jwt.encode(to_encode, SECRET_KEY, alogrithm=ALGORITHM)
    return encode_jwt

def verify_token(token: str):
    try:
        paylaod = jwt.decode(token, SECRET_KEY, algorithm=ALGORITHM)
...
@app.get("/user/me")
def me(
    token: Optional[str] = Depends(oath2_schema),
